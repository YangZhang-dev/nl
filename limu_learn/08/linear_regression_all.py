...
def data_iter(batch_size, features, labels):
    ...
    indices = list(range(num_examples))  # 用下标生成一个列表
    # 这些样本是随机读取的，没有特定的顺序
    random.shuffle(indices)
    for i in range(0, num_examples, batch_size):
        batch_indices = torch.tensor(
            indices[i: min(i + batch_size, num_examples)])
        yield features[batch_indices], labels[batch_indices]  # X.shape=(batch_size,2) y.shape=(batch_size,1)
...
def sgd(params, lr, batch_size):  # @save
    ...
    with torch.no_grad():
        for param in params:
            param -= lr * param.grad / batch_size  # w1=w0-(η*grad/b)
            param.grad.zero_()
